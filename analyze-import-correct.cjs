...
function analyzeFile(filePath) {
  ...
  for (const line of lines) {
    ...
    if (importMatch) {
      ...
      const relativeToSrc = normalize(path.relative(baseDir, fullImportPath));

      for (const { alias, pathPart } of aliasMap) {
        if (relativeToSrc.startsWith(pathPart)) {
          const pathWithoutPrefix = relativeToSrc.slice(pathPart.length).replace(/^\/|\\/, '');
          const suggested = `${alias}/${pathWithoutPrefix}`;
          if (!found) {
            report += `\n📄 File: ${relativePath}\n`;
            found = true;
          }
...
console.log(`✅ Migration report generated: ${normalize(outputPath)}`);
